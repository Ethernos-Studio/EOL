...
- [ ] **先编译出Cavvy的linux可执行文件** - 作为基础，后续平台适配依赖此可执行文件的可移植性测试

#### 0.4.5.x 多平台适配IR代码
- [ ] **目标** - 实现EOL程序在不同平台上的可移植性，无需修改代码
- [ ] **IR 代码适配** - 生成的 IR 代码在不同平台上的兼容性，避免依赖特定平台的指令集
- [ ] **可选生成参数** - 原来的特定平台代码改为可选参数，使用`-f:XX`或`--feature:XX`开启，`-No:XX`关闭，`-D:XX`定义宏，`-U:XX`取消定义宏
- [ ] **目标平台** - 支持 Windows、Linux、macOS 等主要操作系统
- [ ] **支持混淆** - 支持混淆IR代码，防止被反编译和修改
